...
class ModernImageToCSVExtractor:
    def __init__(self, root):
        ...
        self.info_icon = tk.PhotoImage(data=Icon.info)
        self.update_theme()
        ...
        self.detected_cells = []  # Store detected cells for each image
    ...
    def setup_ui(self):
        ...
        header_frame.pack(fill=tb.X, pady=(10, 15))
        tb.Label(header_frame, text="Image to CSV Extractor", bootstyle=INVERSE, font=('Segoe UI', 18, 'bold')).pack(side=tb.LEFT, padx=10)
        tb.Button(header_frame, text="Toggle Dark Mode", bootstyle=SECONDARY, image=self.info_icon, compound=tb.LEFT, command=self.toggle_theme).pack(side=tb.RIGHT, padx=10)
        ...
        control_canvas = tb.Canvas(main_frame, width=300)
        scrollbar = tb.Scrollbar(main_frame, orient=VERTICAL, command=control_canvas.yview)
        control_frame = tb.Frame(control_canvas)

        control_canvas.configure(yscrollcommand=scrollbar.set)

        # Pack the canvas and scrollbar
        control_canvas.pack(side=tb.LEFT, fill=tb.Y)
        scrollbar.pack(side=tb.LEFT, fill=tb.Y)
        ...
        self.display_frame = tb.Frame(main_frame)
        self.display_frame.pack(side=tb.RIGHT, fill=tb.BOTH, expand=True)
        ...
        self.status_var = tb.StringVar(value=f"Ready. Load images to begin. {self.get_current_time()}")
        status_bar = tb.Label(self.root, textvariable=self.status_var, bootstyle=SECONDARY, anchor="w")
        status_bar.pack(side=tb.BOTTOM, fill=tb.X, pady=5)

    def setup_file_section(self, parent):
        file_frame = tb.Labelframe(parent, text="File Management", padding=10, bootstyle=PRIMARY)
        file_frame.pack(fill=tb.X, pady=5)
        tb.Button(file_frame, text="Load Images", bootstyle=SUCCESS, command=self.load_images).pack(fill=tb.X, pady=2)
        self.image_select_var = tb.StringVar()
        self.image_select_combo = tb.Combobox(file_frame, textvariable=self.image_select_var, state="readonly")
        self.image_select_combo.pack(fill=tb.X, pady=2)
        self.image_select_combo.bind("<<ComboboxSelected>>", self.switch_image)
        self.filename_var = tb.StringVar(value="No files selected")
        tb.Label(file_frame, textvariable=self.filename_var, wraplength=280).pack(fill=tb.X, pady=2)

    def setup_processing_section(self, parent):
        process_frame = tb.Labelframe(parent, text="Image Processing", padding=10, bootstyle=PRIMARY)
        process_frame.pack(fill=tb.X, pady=5)
        
        type_frame = tb.Frame(process_frame)
        type_frame.pack(fill=tb.X, pady=2)
        tb.Label(type_frame, text="Process Type:").pack(side=tb.LEFT)
        self.process_type = tb.StringVar(value="original")
        process_combo = tb.Combobox(type_frame, textvariable=self.process_type, values=["original", "threshold", "adaptive", "otsu"],
                                   state="readonly", width=12)
        process_combo.pack(side=tb.RIGHT, fill=tb.X, expand=True)
        process_combo.bind("<<ComboboxSelected>>", self.update_processing)

        thresh_frame = tb.Frame(process_frame)
        thresh_frame.pack(fill=tb.X, pady=2)
        tb.Label(thresh_frame, text="Threshold:").pack(side=tb.LEFT)
        self.threshold_var = tb.IntVar(value=127)
        threshold_slider = tb.Scale(thresh_frame, from_=0, to=255, orient=HORIZONTAL,
                                   variable=self.threshold_var, command=self.update_processing)
        threshold_slider.pack(side=tb.RIGHT, fill=tb.X, expand=True)
        tb.Label(thresh_frame, textvariable=self.threshold_var, width=3).pack(side=tb.RIGHT, padx=5)

        contrast_frame = tb.Frame(process_frame)
        contrast_frame.pack(fill=tb.X, pady=2)
        tb.Label(contrast_frame, text="Contrast:").pack(side=tb.LEFT)
        self.contrast_var = tb.DoubleVar(value=1.0)
        self.contrast_display_var = tb.StringVar(value="1.0")
        ...
        contrast_slider = tb.Scale(contrast_frame, from_=0.5, to=2.5, orient=HORIZONTAL,
                                  variable=self.contrast_var)
        contrast_slider.pack(side=tb.RIGHT, fill=tb.X, expand=True)
        tb.Label(contrast_frame, textvariable=self.contrast_display_var, width=3).pack(side=tb.RIGHT, padx=5)
        
        tb.Button(process_frame, text="Apply Processing", bootstyle=INFO, command=self.update_processing).pack(fill=tb.X, pady=2)

    def setup_selection_section(self, parent):
        selection_frame = tb.Labelframe(parent, text="Table Selection", padding=10, bootstyle=PRIMARY)
        selection_frame.pack(fill=tb.X, pady=5)
        tb.Checkbutton(selection_frame, text="Automatic Table Detection", variable=self.use_auto_detection, bootstyle=INFO, command=self.toggle_auto_detection).pack(anchor=tb.W, pady=2)
        tb.Separator(selection_frame, orient='horizontal').pack(fill=tb.X, pady=2)
        self.selection_mode_var = tb.StringVar(value="rows")
        tb.Radiobutton(selection_frame, text="Select Rows", variable=self.selection_mode_var, value="rows", bootstyle=SUCCESS, command=self.change_selection_mode).pack(anchor=tb.W, pady=2)
        tb.Radiobutton(selection_frame, text="Select Columns", variable=self.selection_mode_var, value="columns", bootstyle=WARNING, command=self.change_selection_mode).pack(anchor=tb.W, pady=2)
        tb.Separator(selection_frame, orient='horizontal').pack(fill=tb.X, pady=2)
        selection_info = tb.Frame(selection_frame)
        selection_info.pack(fill=tb.X, pady=2)
        self.rows_count_var = tb.StringVar(value="Rows: 0")
        self.cols_count_var = tb.StringVar(value="Columns: 0")
        tb.Label(selection_info, textvariable=self.rows_count_var).pack(side=tb.LEFT, padx=5)
        tb.Label(selection_info, textvariable=self.cols_count_var).pack(side=tb.RIGHT, padx=5)
        tb.Button(selection_frame, text="Clear Selections", bootstyle=SECONDARY, command=self.clear_selections).pack(fill=tb.X, pady=2)

    def setup_export_section(self, parent):
        export_frame = tb.Labelframe(parent, text="Data Export", padding=10, bootstyle=PRIMARY)
        export_frame.pack(fill=tb.X, pady=5)
        tb.Button(export_frame, text="Preview Data", bootstyle=INFO, command=self.preview_data).pack(fill=tb.X, pady=2)
        tb.Button(export_frame, text="Export to CSV", bootstyle=DANGER, command=self.export_to_csv).pack(fill=tb.X, pady=2)
        tb.Button(export_frame, text="Help", bootstyle=SECONDARY, command=self.show_help).pack(fill=tb.X, pady=2)
    ...
    def setup_display_panel(self):
        ...
        self.canvas.draw()
        self.canvas.get_tk_widget().pack(fill=tb.BOTH, expand=True)
    ...
    def load_images(self):
        ...
        if file_paths:
            try:
                ...
                self.selected_rows = []
                self.selected_columns = []
                self.detected_cells = []  # Reset detected cells
    ...
    def detect_table_structure(self):
        if not self.images_cv or self.current_image_index >= len(self.images_cv):
            messagebox.showwarning("Warning", "Please load an image first.")
            return
        try:
            current_image = self.images_cv[self.current_image_index]
            tables = self.table_detector.detect_tables(current_image)
            print(f"Tables detected: {len(tables)}")  # DEBUG
            if tables:
                print(f"First table cells: {len(tables[0]['cells'])}")  # DEBUG
            if not tables:
                messagebox.showinfo("Info", "No tables detected in the image.")
                self.detected_cells[self.current_image_index] = []
                return
            table = tables[0]
            self.selected_rows = []
            self.selected_columns = []
            # Store detected cells for this image
            self.detected_cells[self.current_image_index] = [cell['bbox'] for cell in table['cells']]
            self.update_image_display()
            self.ax.clear()
            self.ax.imshow(self.processed_images[self.current_image_index])
            x, y, w, h = table['bbox']
            ...
            for cell in table['cells']:
                ...
                cell_rect = patches.Rectangle((cx, cy), cw, ch, linewidth=1, edgecolor='g', facecolor='none')
                self.ax.add_patch(cell_rect)
            self.canvas.draw()
            self.status_var.set(f"Table detected with {len(table['cells'])} cells. {self.get_current_time()}")
        except Exception as e:
            messagebox.showerror("Error", f"Error detecting table structure: {str(e)}")
            self.status_var.set(f"Error detecting table structure. {self.get_current_time()}")
    ...
    def extract_data(self):
        ...
        try:
            for idx, processed_image in enumerate(self.processed_images):
                # Use detected cells if available and auto-detection is enabled
                use_cells = self.use_auto_detection.get() and self.detected_cells and self.detected_cells[idx]
                table_data = []
                if use_cells:
                    # Sort cells top-to-bottom, then left-to-right
                    cells = self.detected_cells[idx]
                    # Get all cell bboxes and sort by y, then x
                    sorted_cells = sorted(cells, key=lambda b: (b[1], b[0]))
                    # Try to group cells into rows by y coordinate
                    ...
                    for cell in sorted_cells:
                        ...
                        if not placed:
                            rows.append([cell])
                    # For each row, sort cells left-to-right
                    for row in rows:
                        row = sorted(row, key=lambda b: b[0])
                        row_data = []
                        for x, y, w, h in row:
                            cell_img = processed_image[y:y+h, x:x+w]
                            if cell_img.size == 0:
                                row_data.append("")
                                continue
                            pil_cell_img = Image.fromarray(cell_img)
                            # Preprocess: thresholding for better OCR
                            cell_gray = cv2.cvtColor(np.array(pil_cell_img), cv2.COLOR_RGB2GRAY)
                            _, cell_bin = cv2.threshold(cell_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                            pil_cell_img = Image.fromarray(cell_bin)
                            text = pytesseract.image_to_string(pil_cell_img, lang=self.config['lang'], config='--psm 7').strip()
                            text = text.replace('\n', ' ').replace('\r', ' ').strip()
